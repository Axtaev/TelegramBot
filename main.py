...
async def generate_compliment():
    response = await client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "Ты генератор комплиментов для девушки по имени Азиза. Комплименты должны быть женственными, искренними, оригинальными и с эмодзи. И пусть они будут всегда милыми и вдохновляющими, а самое главное пусть они не повторяются."},
